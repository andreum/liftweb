  ...
  */
  ...
  def save(toSave: A): Boolean = {
    ...
    def runAppliers(rs: ResultSet) : Boolean = {
      try {
        if (rs.next) {
          // val meta = rs.getMetaData - crashes on Oracle ReturnResultSetImpl
          toSave.runSafe {
    ...
    if (conn.returningIntoClause_?) {
      for (col <- mappedColumns) {
	      if (columnPrimaryKey_?(col._1)) {
          conn.driverType.registerReturnParameter(st,colNum,
            col._2.invoke(toSave, null).asInstanceOf[MappedField[AnyRef, A]]
      ...
      colNum = colNum + 1
          ...
          val oneRowUpdated : Boolean = (conn.brokenAutogeneratedKeys_?, postQuery, indexMap) match {
            ...
            case (true, _, _)     =>  runAppliers(conn.driverType.executeReturning(st))
        ...
        val ret = if (conn.wickedBrokenAutogeneratedKeys_?) {
          DB.prepareStatement(query, conn) {
            ...
            val pks = mappedColumnInfo.filter(_._2.dbPrimaryKey_?)
            val pkName = ((pks.map(_._1)).toList).mkString(",")
            val intoClause = if (conn.returningIntoClause_?){
              " INTO "+pks.map(x=>"?").toList.mkString(",")
            } else ""
             DB.prepareStatement(query + " RETURNING " + pkName+intoClause, conn) {
